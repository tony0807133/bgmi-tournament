  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="card text-center py-12 text-gray-500">No transactions</div>
        ) : filtered.map(tx => (
...
export default function Wallet() {
  ...
  const fetchData = async () => {
    try {
      const [w, wd, ref] = await Promise.all([
  ...
  const handleDeposit = async (e) => {
    ...
    try {
      ...
      const options = {
        ...
        name: 'BGMI Arena', description: 'Wallet Deposit', order_id: order.id,
  ...
  const handleWithdraw = async (e) => {
    ...
    setWithdrawing(true);
    try {
      await axios.post('/api/wallet/withdraw', { amount: Number(withdrawForm.amount), upiId: withdrawForm.upiId });
  ...
  if (fetchError) return (
    <div className="max-w-2xl mx-auto px-4 py-32 text-center">
      ...
      <button onClick={() => { setFetchError(false); setLoading(true); fetchData().finally(() => setLoading(false)); }} className="btn-primary">Try Again</button>
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="relative overflow-hidden card mb-6 py-10 text-center border-orange-500/20">
        ...
        <p className="text-6xl font-black"><span className="text-gray-400 text-3xl">₹</span><span className="text-orange-400">{data.wallet}</span></p>
        <p className="text-gray-600 text-xs mt-2">Prizes are credited to team leader's wallet</p>
      </div>

      {/* Referral card */}
      {referral?.referralCode && (
        <div className="card mb-4 border-purple-500/20 bg-gradient-to-br from-purple-500/10 to-transparent">
          <h2 className="font-bold mb-1 flex items-center gap-2 text-purple-400"><span>🎁</span> Refer & Earn</h2>
          <p className="text-xs text-gray-500 mb-3">Share your code — both you and your friend get ₹20 wallet bonus</p>
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 font-mono font-black text-lg tracking-widest text-purple-300">
              {referral.referralCode}
            </div>
            <button onClick={() => { navigator.clipboard.writeText(referral.referralCode); toast.success('Copied!'); }}
              className="btn-secondary px-4 py-2.5 text-sm">Copy</button>
          </div>
      ...
      <div className="card mb-4 border-green-500/10">
        <h2 className="font-bold mb-1 flex items-center gap-2 text-green-400"><span>💳</span> Add Money to Wallet</h2>
          ...
          {QUICK_AMOUNTS.map(a => (
            <button key={a} type="button" onClick={() => setDepositAmount(String(a))}
              className={`px-3 py-1.5 rounded-xl text-sm font-semibold border transition-all ${
                depositAmount === String(a) ? 'bg-green-500 border-green-500 text-white' : 'bg-white/5 border-white/10 text-gray-400 hover:border-green-500/40 hover:text-white'
              }`}>₹{a}</button>
          ))}
        </div>
        <form onSubmit={handleDeposit} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input className="input pl-8" type="number" min="10" step="1" placeholder="Enter amount" value={depositAmount} onChange={e => setDepositAmount(e.target.value)} required />
          </div>
          <button type="submit" className="btn-success px-6 py-2.5 whitespace-nowrap" disabled={depositing}>
            {depositing ? <span className="flex items-center gap-2"><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Processing...</span> : '+ Add Money'}
          </button>
        </form>
      </div>
      ...
      <div className="card mb-6">
        <h2 className="font-bold mb-1 flex items-center gap-2"><span>💸</span> Withdraw to UPI</h2>
        ...
        <form onSubmit={handleWithdraw} className="space-y-3">
          <div>
            ...
            <input className="input" placeholder="yourname@paytm / name@upi" value={withdrawForm.upiId} onChange={e => setWithdrawForm({ ...withdrawForm, upiId: e.target.value })} required />
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5 block">Amount</label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold">₹</span>
              <input className="input pl-8" type="number" min="10" max={data.wallet} placeholder="0" value={withdrawForm.amount} onChange={e => setWithdrawForm({ ...withdrawForm, amount: e.target.value })} required />
            </div>
      ...
      <div className="flex gap-2 mb-4">
        {['transactions', 'withdrawals'].map(t => (
          <button key={t} onClick={() => setTab(t)} className={`tab-btn capitalize ${tab === t ? 'tab-active' : 'tab-inactive'}`}>
            {t}<span className="ml-1.5 text-xs opacity-60">({t === 'transactions' ? data.transactions.length : withdrawals.length})</span>
          </button>
        ))}
      </div>

      {tab === 'transactions' && <TransactionList transactions={data.transactions} />}
        ...
        <div className="space-y-2">
          {withdrawals.length === 0 ? (
            ...
            <div key={w._id} className="card py-3.5">
              ...
              {w.adminNote && <p className="text-xs text-gray-400 mt-1 bg-white/3 rounded-lg px-2 py-1">Note: {w.adminNote}</p>}
            </div>
          ))}
        </div>
      )}
