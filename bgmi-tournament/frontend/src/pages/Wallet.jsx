...
const upiQr = (amt) => {
  const str = amt
    ? `upi://pay?pa=${ADMIN_UPI_ID}&pn=${encodeURIComponent(ADMIN_UPI_NAME)}&am=${amt}&cu=INR`
...
export default function Wallet() {
  ...
  const fetchData = async () => {
    try {
      const [w, dep, wd, ref] = await Promise.all([
  ...
  const handleWithdraw = async (e) => {
    e.preventDefault();
    if (Number(wForm.amount) < 10) return toast.error('Minimum ₹10');
    setWithdrawing(true);
    try {
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="relative overflow-hidden rounded-2xl mb-6 p-8 text-center border border-orange-500/20 bg-gradient-to-br from-orange-500/10 via-transparent to-transparent">
        ...
        <p className="text-7xl font-black mt-2">
          <span className="text-gray-400 text-4xl align-top mt-3 inline-block">₹</span>
      ...
      {referral?.referralCode && (
        <div className="card mb-5 border-purple-500/20 bg-gradient-to-br from-purple-500/8 to-transparent">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="font-bold text-purple-400">🎁 Refer & Earn ₹20</p>
              <p className="text-xs text-gray-500 mt-0.5">You and your friend both get ₹20 on signup</p>
            </div>
      ...
      <div className="card mb-5 border-green-500/15">
        ...
        {step === 0 && (
          <div className="space-y-4">
            ...
            <button onClick={() => { if (!amount || Number(amount) < 10) { toast.error('Minimum ₹10'); return; } setStep(1); }}
              ...
              <div className="flex flex-col sm:flex-row items-center gap-5">
                <div className="shrink-0 flex flex-col items-center gap-2">
                  <div className="w-[140px] h-[140px] rounded-2xl overflow-hidden bg-white p-2 shadow-xl">
                    <img src={upiQr(amount)} alt="UPI QR" className="w-full h-full object-contain" />
                  </div>
                ...
                <div className="flex-1 w-full space-y-3">
                  <div className="bg-white/5 rounded-xl p-3 border border-white/8">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-black text-orange-400 text-sm break-all">{ADMIN_UPI_ID}</p>
                      <button onClick={() => { navigator.clipboard.writeText(ADMIN_UPI_ID); toast.success('Copied!'); }}
                  ...
                  <div className="bg-white/5 rounded-xl p-3 border border-white/8">
                    ...
                    <p className="font-black text-2xl text-green-400">₹{amount}</p>
                  </div>
                  <div className="bg-white/5 rounded-xl p-3 border border-white/8">
                    <p className="text-xs text-gray-500 mb-1 uppercase tracking-wider">Account Name</p>
                    <p className="font-semibold text-white text-sm">{ADMIN_UPI_NAME}</p>
                  </div>
                </div>
              </div>
              <div className="mt-4 flex items-start gap-2 bg-yellow-500/8 border border-yellow-500/20 rounded-xl p-3">
                <span className="text-yellow-400 text-sm mt-0.5">⚠</span>
                <p className="text-xs text-yellow-300">Pay exactly <span className="font-black">₹{amount}</span>. Do not close this page after paying.</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setStep(0)} className="btn-secondary flex-1 py-3">← Back</button>
              <button onClick={() => setStep(2)} className="btn-primary flex-1 py-3">I've Paid → Upload Proof</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="bg-green-500/8 border border-green-500/20 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-green-400">Payment of ₹{amount} done?</p>
            ...
            <input className="input" type="text" placeholder="UTR / Transaction ID (optional — helps faster approval)"
      ...
      <div className="card mb-6 border-white/8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Min ₹10 · Sent to your UPI within 24 hrs</p>
          </div>
        </div>
        <form onSubmit={handleWithdraw} className="space-y-3">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input className="input pl-8" type="number" min="10" max={data.wallet} placeholder="0"
                value={wForm.amount} onChange={e => setWForm({ ...wForm, amount: e.target.value })} required />
            </div>
            <p className="text-xs text-gray-600 mt-1">Available: <span className="text-orange-400 font-bold">₹{data.wallet}</span></p>
          </div>
            ...
            : deposits.map(d => (
              <div key={d._id} className="card py-3.5">
                <div className="flex items-center justify-between mb-1.5">
                  <p className="font-black text-lg text-green-400">+₹{d.amount}</p>
